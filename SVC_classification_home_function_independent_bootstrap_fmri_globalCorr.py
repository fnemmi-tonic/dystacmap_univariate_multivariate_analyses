...
#select only globalCorr values
globalCorr_values = data.iloc[:,0:14]
features_name = data.columns[0:14]
...
globalCorr_val_nuisance_removed = np.empty([len(globalCorr_values), len(globalCorr_values.columns)])
for n,region in enumerate(globalCorr_values.columns):
    regr.fit(nuisance, globalCorr_values.loc[:,region])
    pred = regr.predict(nuisance)
    res = globalCorr_values.loc[:,region] - pred
    globalCorr_val_nuisance_removed[:,n] = res
    

globalCorr_val_nuisance_removed_norm_no_td = globalCorr_val_nuisance_removed[label != 1, :]
label_no_td = label[label != 1]
subj_id_no_td = np.arange(globalCorr_val_nuisance_removed_norm_no_td.shape[0])

real_mean_no_td, real_std_no_td, shuffled_mean_no_td, shuffled_std_no_td, real_otp_no_td, shuffled_otp_no_td, gw_no_td, shuff_gw_no_td, bal_no_td, bal_no_td_shuff, best_comb = SVC_real_and_shuffled_bootstrap_feat_select(globalCorr_val_nuisance_removed_norm_no_td, 
...
with open("globalCorr_results_100.pkl", "wb") as f:
