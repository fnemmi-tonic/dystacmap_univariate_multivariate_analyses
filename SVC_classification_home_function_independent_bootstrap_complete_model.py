...
#gm
data = read_csv("gm_value_arranged_by_subject.csv")
...
label_no_td_gm = label[label != 1]
subj_id_no_td_gm = np.arange(gm_val_nuisance_removed_norm_no_td.shape[0])


#wm
data = read_csv("wm_value_arranged_by_subject.csv")
#select only gm values
wm_values = data.iloc[:,0:6]
features_name_wm = data.columns[0:6]
subjects = data.iloc[:,6]
label = data.loc[:,"Group"]
nuisance = data.loc[:,["pre", "aix", "TIV", "qa"]]
...
wm_val_nuisance_removed = np.empty([len(wm_values), len(wm_values.columns)])
for n,region in enumerate(gm_values.columns):
    regr.fit(nuisance, gm_values.loc[:,region])
    pred = regr.predict(nuisance)
    res = gm_values.loc[:,region] - pred
...
wm_val_nuisance_removed_norm_no_td = wm_val_nuisance_removed[label != 1, :]
label_no_td_wm = label[label != 1]
subj_id_no_td_wm = np.arange(wm_val_nuisance_removed_norm_no_td.shape[0])


#fmri indexes
data = read_csv("globalCorr_value_arranged_by_subject.csv")
...
data = read_csv("localCorr_value_arranged_by_subject.csv")
localCorr_values = data.iloc[:,0:22]
features_name_localCorr = data.columns[0:22]
data = read_csv("fALFF_value_arranged_by_subject.csv")
...
features_name = ["gm_" + str(el) for el in features_name_gm] + ["wm_" + str(el) for el in features_name_wm] + ["globalCorr_" + str(el) for el in features_name_falff] + ["localCorr_" + str(el) for el in features_name_localCorr] + ["falff_" + str(el) for el in features_name_globalCorr]
...
label_no_td_fmri = label[label != 1]
label_no_td = label[label != 1]
subj_id_no_td_fmri = np.arange(all_indexes_val_nuisance_removed_norm_no_td.shape[0])
subj_id_no_td = np.arange(all_indexes_val_nuisance_removed_norm_no_td.shape[0])


complete_data = np.hstack((gm_val_nuisance_removed_norm_no_td, wm_val_nuisance_removed_norm_no_td, all_indexes_val_nuisance_removed_norm_no_td))

real_mean_no_td, real_std_no_td, shuffled_mean_no_td, shuffled_std_no_td, real_otp_no_td, shuffled_otp_no_td, gw_no_td, shuff_gw_no_td, bal_no_td, bal_no_td_shuff, best_comb = SVC_real_and_shuffled_bootstrap_feat_select(complete_data, 
...
with open("complete_results_100.pkl", "wb") as f:
